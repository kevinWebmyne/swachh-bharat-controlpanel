...
const LanguageEdit = () =>{
    ...
    useEffect(() => {
        ...
        const getSingleProductData = async () => {
            const { data } = await axios.get(`http://localhost:3001/api/language/${id}`, {
            headers: {authorization:"Bearer " + auth},
        })
            console.log(data.data.languageCode)
            SetLanguageCode(data.data.languageCode)
            SetLanguageName(data.data.languageName)
    ...
    const Submit = async (e) => {
        ...
        await axios.put(`http://localhost:3001/api/language/${id}`, data, {
            headers: {authorization:"Bearer " + auth},
        }).then(result => {
            console.log(result);
