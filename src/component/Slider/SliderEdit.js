...
const SliderEdit = () => {
    ...
    useEffect(() => {
        ...
        const getSingleProductData = async () => {
            const { data } = await axios.get(`http://localhost:3001/api/slider/${id}`)
            console.log(data.data.status)
            SetSliderTitle(data.data.sliderTitle)
    ...
    const Submit = async (e) => {
        ...
        await axios.put(`http://localhost:3001/api/slider/${id}`, formdata,
            {
            ...
            <Box component="main" sx={{ flexGrow: 1, p: 3, width: { sm: `calc(100% - ${drawerWidth}px)` } }} style={{ marginLeft: 250, backgroundColor: '#ecf0f5' }}>
                ...
                <div style={{ backgroundColor: 'white', height: 1000 }}>
                    ...
                    <div style={{ display: 'inline-flex', paddingTop: 60, paddingLeft: 60 }}>
                        ...
                        <input type='text' onChange={(e) => { SetSliderTitle(e.target.value) }} style={{ marginLeft: 40, height: 35, width: 300, textAlign: 'center' }} value={sliderTitle} placeholder='State Title' />
