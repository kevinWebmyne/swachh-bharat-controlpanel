...
const CountryView = () => {
    ...
    const getProductsData = async () => {
        const { data } = await axios.get(`http://localhost:3001/api/noAuth/country/?page=${page}&size=${size}`, {
    ...
    const deleteHandle =  (id) => {
        ...
        let body = "";
            ...
            if (window.confirm(text) == true) {
                console.log(auth);
                 axios.put(`http://localhost:3001/api/country/delete/${id}`, body  ,{
                    headers: {authorization:"Bearer " + auth }
                }
                ).then(result=>{
            ...
            <Box component="main" sx={{ flexGrow: 1, p: 3, width: { sm: `calc(100% - ${drawerWidth}px)` } }} style={{ marginLeft: 250, backgroundColor: '#ecf0f5' }}>
                ...
                <div style={{ backgroundColor: 'white', height: 1000 }}>
                    <div style={{ backgroundColor: 'white', display: 'inline-flex', marginTop: 15 }}>
                        ...
                        <Link to={'/countryAdd'}>     
                        <button style={{ color: 'black',height:30, backgroundColor: 'lightgreen', borderStyle: 'none', marginLeft: 780, width: 100 }}> Add Country</button>
                        </Link>
                    </div>
                    <br />
                    <hr />
                        ...
                        <tbody>
...

                            {
                                items.map((product, t) =>
                                    <tr key={t}>
                                        <td>{t + 1}</td>
                                        <td>{product.countryTitle}</td>
                                        <td>{product.isdnCode}</td>
                                         <td>
                  <Link to={`/country/${product._id}`}>
