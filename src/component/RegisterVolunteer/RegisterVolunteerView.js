...
const RegisterVolunteerView = () => {
    ...
    const getProductsData = async () => {
        const { data } = await axios.get(`http://localhost:3001/api/noAuth/volunteers/?page=${page}&size=${size}`, {
    ...
    const deleteHandle =  (id) => {
        ...
        let body = "";
            ...
            if (window.confirm(text) == true) {
                console.log(auth);
                 axios.put(`http://localhost:3001/api/volunteers/delete/${id}`, body  ,{
                    headers: {authorization:"Bearer " + auth }
                }
                ).then(result=>{
